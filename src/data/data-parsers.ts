...
export class DataParser {
  public static getParser(entity: HassEntity): Parser {
    ...
    if (HaDeparturesEntityAttributes.TIMES in entity.attributes || HaDeparturesEntityAttributes.PLANNED_DEPARTURE_TIME in entity.attributes) {
      ...
      console.debug("Detected 'db-infoscreen' attribute(s)", entity.attributes);
  ...
  public getTimes(): Array<DepartureTime> {
    ...
    if (this._entity.attributes[HaDeparturesEntityAttributes.TIMES]) {
      this._entity.attributes[HaDeparturesEntityAttributes.TIMES].forEach((timeEntry: any) => {
        ...
        const hasAlerts: boolean = timeEntry.alerts === true;

        times.push(new DepartureTime(timePlanned, timeEstimated, tripId, headSign, cancelled, hasAlerts));
      });
