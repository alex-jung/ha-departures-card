 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */const _t="important",gt=" !"+_t,yt=(t=>(...e)=>({_$litDirective$:t,values:e}))(class extends $t{constructor(t){if(super(t),t.type!==ft||"style"!==t.name||t.strings?.length>2)throw Error("The `styleMap` directive must be used in the `style` attribute and must be the only part in the attribute.")}render(t){return Object.keys(t).reduce(((e,s)=>{const i=t[s];return null==i?e:e+`${s=s.includes("-")?s:s.replace(/(?:^(webkit|moz|ms|o)|)(?=[A-Z])/g,"-$&").toLowerCase()}:${i};`}),"")}update(t,[e]){const{style:s}=t.element;if(void 0===this.ft)return this.ft=new Set(Object.keys(e)),this.render(e);for(const t of this.ft)null==e[t]&&(this.ft.delete(t),t.includes("-")?s.removeProperty(t):s[t]=null);for(const t in e){const i=e[t];if(null!=i){this.ft.add(t);const e="string"==typeof i&&i.endsWith(gt);t.includes("-")||e?s.setProperty(t,e?i.slice(0,-11):i,e?_t:""):s[t]=i}}return F}});let mt=class extends rt{render(){const t={width:this.config.showTransportIcon?"100px":"70px"};return B`
        <div>
        ...
        `}renderDepartureTime(){return B`
            ${this._departure}
            ${this.renderDelay()}
        `}renderDelay(){if(!this.showDelay||this._now)return Y;const t=this._delay>0,e=this._delay>=0?"+":"-";return B`<span class="delay" ?delayed=${t}>(${e}${this._delay})</span>`}};te.styles=[ut,o`
        :host {
            display: flex;
            flex-wrap: nowrap;
            align-items: center;
